...
resource "hyperv_iso_image" "bootstrap" {
  ...
  destination_iso_file_path = "c:\\Archive\\VirtualBoxOS\\debian-12.5.0-amd64-netinst.iso"
...
resource "hyperv_vhd" "web_server_g2_vhd" {
  path = "c:\\ProgramData\\Microsoft\\Windows\\Virtual Hard Disks\\web_server_g2.vhdx" #Needs to be absolute path
...
resource "hyperv_machine_instance" "default" {
  ...
  dvd_drives {
    ...
    //path = ""
    path               = hyperv_iso_image.bootstrap.destination_iso_file_path
    resource_pool_name = ""
  }
